#!/usr/bin/env python3
"""
Extract transactions in statement order with correct cardholder assignment
Based on the logic: cardholder name appears one line before "TRANSACTIONS THIS CYCLE"
"""

import csv
import re

def extract_transactions_statement_order():
    """Extract transactions in the exact order they appear in the statement"""
    
    # This is the exact transaction data from the PDF in order
    statement_text = """
...
"""
    ...
    while i < len(lines):
        ...
        if i + 1 < len(lines) and "TRANSACTIONS THIS CYCLE" in lines[i + 1]:
            ...
            # Skip the "TRANSACTIONS THIS CYCLE" line
            i += 2
            continue
...
def main():
    print("="*60)
    print("EXTRACTING TRANSACTIONS IN STATEMENT ORDER")
    print("="*60)
    
    # Extract transactions in statement order
    transactions = extract_transactions_statement_order()
    
    # Save to CSV
    filename = 'chase_statement_order_transactions.csv'
    ...
    # Show first few transactions to verify order and assignment
    print(f"\nFirst 10 transactions (to verify order and cardholder assignment):")
    for i, txn in enumerate(transactions[:10]):
        print(f"{i+1:2d}. {txn['date']} - {txn['cardholder']} - {txn['merchant'][:40]:40} - ${txn['amount']:>8.2f}")
